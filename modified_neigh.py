# -*- coding: utf-8 -*-
"""modified-neigh.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dAp3WUQKFYpJEEjsme-3pT9fgiwebUKj
"""
...
class Packet:
  def __init__(self, packet_id, ring):
    ...
    self.AoI = 0 #  Age of Information of the packet

max_AoI = 0
arxi=time.time()
B = 400 #  bound for AoI
drop = 0 # number of dropped packets so far
...
while (times<=5000):
  ...
  for i in ring:
    ...
    if max_AoI == -1:
      ...
      if best.destination in i.neighbors:
        #print(f"I am node {i.id} and I offer packet {best.id} to my neighboor {best.destination.id} which is the final destination")
        success = success + 1
        best.custod = best.destination
        best.delivered = True
      else: 
        max_value=-2
        kaliteros=-2
        for k in i.neighbors:
          if k.vector[best.destination.id] > max_value:
            max_value = k.vector[best.destination.id]
            kaliteros = k
        if kaliteros != -2:
          #print(f"I am node {i.id} and I give packet {best.id} to my best neighbor {k.id}")
          best.custod = kaliteros
          best.flag = True
  for i in packets:
    if i.delivered == False:
      i.AoI = i.AoI + 1
      if i.AoI == B:
        i.delivered = True
        #print(f"Packet {i.id} dropped because of big AoI")
        drop = drop + 1
    if i.flag == True:
      i.flag = False
  times=times+1
  #for i in packets:
    #if i.delivered == False:
      #print(f"Packet {i.id} has Age {i.AoI}")
total = 0
for i in packets:
  total = total + i.AoI
  if i.AoI > max_AoI:
...
print(f"Total number of dropped packets: {drop}")
print(f"Total number of successfull transmissions: {success}")
print(f"Total number of remaining packets in the network: {id-success-drop}")
telos=time.time()
